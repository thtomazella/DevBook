...
// SeguirUsuario permite que um usuarios siga outro
...
func PararDeSeguirUsuario(w http.ResponseWriter, r *http.Request) {
	...
	if seguidorID == usuarioID {
		respostas.Erro(w, http.StatusForbidden, errors.New("Não é possível deixar de seguir vocẽ mesmo"))
	}
...
// BuscarSeguindo permite buscar os usuarios que um determinando usuario está seguindo
...
func AtualizarSenha(w http.ResponseWriter, r *http.Request) {
	...
	if usuarioIDNoToken != usuarioID {
		respostas.Erro(w, http.StatusForbidden, errors.New("Não é possível atualizar a senha de uma usuário diferente do seu"))
	}
	corpoRequisicao, erro := ioutil.ReadAll(r.Body)
	...
	if erro = seguranca.VerificarSenha(senhaSalvaNoBanco, senha.Atual); erro != nil {
		respostas.Erro(w, http.StatusInternalServerError, errors.New("A Senha atual não codiz com a que está atual"))
		return
	}
